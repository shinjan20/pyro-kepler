...
const WorkingCandidateView = ({ candidate, onClose, onMessage, onSendLetter }: WorkingCandidateViewProps) => {
    ...
    const handleSend = () => {
        if (letterModalOpen && letterContent.trim()) {
            ...
            alert(`${letterModalOpen === 'joining' ? 'Joining' : 'Completion'} letter sent successfully directly to their Messages!`);
        }
    };
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden animate-in fade-in duration-300">
            ...
            <div className="bg-gradient-to-br from-brand-600 to-purple-700 p-8 text-white relative">
                ...
                <div className="relative z-10 flex items-start gap-6">
                    ...
                    <div className="flex-1 pt-2">
                        ...
                        <div className="mt-4 inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full text-sm font-bold border border-white/30">
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                <button
                    ...
                    className="flex flex-col items-center justify-center gap-3 p-6 bg-slate-50 dark:bg-slate-800/50 hover:bg-brand-50 hover:text-brand-600 hover:border-brand-200 dark:hover:bg-brand-900/20 dark:hover:border-brand-800 border border-slate-200 dark:border-slate-700 rounded-2xl transition-all group"
                >
                    <div className="p-4 bg-white dark:bg-slate-800 rounded-full shadow-sm group-hover:scale-110 transition-transform">
                        <MessageSquare className="w-6 h-6" />
                    </div>
                    <span className="font-bold">Message Candidate</span>
                </button>
                <a
                    href={`mailto:dummy@example.com?subject=Regarding Live Project`}
                    className="flex flex-col items-center justify-center gap-3 p-6 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-slate-300 border border-slate-200 dark:border-slate-700 rounded-2xl transition-all group"
                >
                    <div className="p-4 bg-white dark:bg-slate-800 rounded-full shadow-sm group-hover:scale-110 transition-transform text-slate-700 dark:text-slate-300">
                        <Mail className="w-6 h-6" />
                    </div>
                    <span className="font-bold text-slate-700 dark:text-slate-300">Email Candidate</span>
                </a>

                <button
                    onClick={() => openLetterModal('joining')}
                    className="flex flex-col items-center justify-center gap-3 p-6 bg-brand-50 dark:bg-brand-900/20 hover:bg-brand-100 hover:border-brand-300 dark:hover:bg-brand-900/40 border border-brand-200 dark:border-brand-800/50 rounded-2xl transition-all group text-brand-600 dark:text-brand-400"
                >
                    <div className="p-4 bg-white dark:bg-slate-900 rounded-full shadow-sm group-hover:scale-110 transition-transform">
                        <FileText className="w-6 h-6" />
                    </div>
                    <span className="font-bold">Send Joining Letter</span>
                </button>

                <button
                    onClick={() => openLetterModal('completion')}
                    className="flex flex-col items-center justify-center gap-3 p-6 bg-purple-50 dark:bg-purple-900/20 hover:bg-purple-100 hover:border-purple-300 dark:hover:bg-purple-900/40 border border-purple-200 dark:border-purple-800/50 rounded-2xl transition-all group text-purple-600 dark:text-purple-400"
                >
                    <div className="p-4 bg-white dark:bg-slate-900 rounded-full shadow-sm group-hover:scale-110 transition-transform">
                        <CheckCircle className="w-6 h-6" />
                    </div>
                    <span className="font-bold">Send Completion Letter</span>
                </button>
            </div>
                    ...
                    <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 mt-1">This will be sent directly to the candidate via ProjectHub Messages.</p>
                            </div>
